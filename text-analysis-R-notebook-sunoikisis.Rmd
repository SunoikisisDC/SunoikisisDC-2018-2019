...
Somer notable disadvantages:

- It is slow with large data sets (in which case you might want a server instantiation of RStudio).

- Its programming paradigm is somewhat non-standard. It is based on the commercial [S and Splus language](https://en.wikipedia.org/wiki/S_(programming_language))). 
...
R is an object oriented programming language, so practically everything can be stored as an R object. To make our calculations effective, we need to store these kinds of calculations in variables. Variables can be assigned with either `<-` or `=`. Let's do that, and let's compare the price of a 5-day trip to Seattle to a 7-day trip to Paris.
...
Above you can see how powerful even simple R programming can be: you can store mathemtical operations in named variables and then use those variables to work with other variables (this becomes very important in word frequency calculations). You can also plot the results for quick assessment.
...
The best way to load text files is with the `scan` function. First, download a text file of Dickens's [*Great Expectations*](https://www.dropbox.com/s/qji9ueb46ajait9/great-expectations.txt?dl=0).
...
dickens.lower.v <- tolower(dickens.v) # this makes the whole text lowercased

dickens.words <- strsplit(dickens.lower.v, "\\W") # strsplit is very important: it takes each word in the lowercased words vector and puts each word in a list by finding non-words, i.e., word boundaries
...
dickens.words.v[1:10] # find the first 10 ten words in Great Expectations

```

Did you notice the "\\W" in the `strsplit` argument? What is that again?
...
```{r}
dickens.words.v[1:10]
```


Viola! We might want to examine how many times the third result "father" occurs (one assumes that will be an important word in this book).

```{r}
length(dickens.words.v[which(dickens.words.v=="father")])
```

Or how many unique words there are in the book.

```{r}
length(unique(dickens.words.v))
```

Divide this by the amount of words in the whole book and you can calculate vocabulary density ratios.

```{r}
unique.words <- length(unique(dickens.words.v))

total.words <- length(dickens.words.v)
...
Note that the squiggly line (~) tells the system to return to the root directory, and your Desktop should be the next step (/) from the root.

Now we will load the necessary libraries for these lessons.
...
## Conclusion: Publish your results

How can you adapt these results to other texts?
